...
const Coins = () => {
  ...
  const currencySymbol = currency === "usd" ? "$" : currency === "eur" ? "€" : "NA";
  ...
  return (
    <Container maxW={"container.xl"} >
      ...
      <HStack w={"full"} overflowX={"auto"} p={"8"}>
        ...
        {btns.map((i, index) => (
           <Button key={index} bgColor={"blackAlpha.900"} color={"white"} css={{"&:hover":{ backgroundColor:"#fff", color:"#000000", border: "1px solid #000000"}}} onClick={() => changePage(index + 1)}>{index + 1}</Button>
        ))}
